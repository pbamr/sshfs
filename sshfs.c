/*
  ...
  -------------------------------------------------------------------------------------------------------------------------------
  NEW:
  2023/03 Peter Boettcher, Germany, <peter.boettcher@gmx.net>


  Date          : 2023.03.28

  TEST          : Linux Kernel 6.2
                  Lenovo X230, T460, Dell/Wyse 3040
  ...
  My extension can be found under "NEW:"! Search "NEW:"
  The extensions are: "chmod FILE" and "chmod DIRECTORIES"
...
/* NEW:2023.03.28 Peter Boettcher, Germany */
#define CHMOD_NOT_ACTIVE 7000
/* NEW 2023.03.28 END */
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
	SSHFS_OPT("chmod_file=%o",  chmod_file, 0),
	SSHFS_OPT("chmod_dir=%o",  chmod_dir, 0),
/* NEW END */
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
	/* compile warning:  fi is may used be uninit.. */
	struct fuse_file_info *fi;
/* NEW: 2023.03.28 END */
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
	if (sshfs.chmod_dir != CHMOD_NOT_ACTIVE) {
		sshfs_chmod(path, sshfs.chmod_dir, fi);
	}
/* NEW: 2023.03.28 END */
	//mode = 02770;
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
	...
	RET_VAL = sshfs_open_common(path, mode, fi);

	if (sshfs.chmod_file != CHMOD_NOT_ACTIVE) {
		sshfs_chmod(path, sshfs.chmod_file, fi);
	}
...
/* NEW:2023.03.29, Peter Boettcher, Germany */
/* Not use! Test!*/
/*	switch (stbuf->st_mode & S_IFMT) {
		case S_IFDIR	:	if (sshfs.chmod_dir != CHMOD_NOT_ACTIVE) {
						sshfs_chmod(path, sshfs.chmod_dir, fi);
					}
					break;
			
		case S_IFREG	:	if (sshfs.chmod_file != CHMOD_NOT_ACTIVE) {
						sshfs_chmod(path, sshfs.chmod_file, fi);
					}
					break;
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
"    -o chmod_file=         (660 etc.)\n"
"    -o chmod_dir=          (2770 etc.)\n"
/* NEW:2023.03.28 END */
...
/* NEW:2023.03.28, Peter Boettcher, Germany */
	sshfs.chmod_file = CHMOD_NOT_ACTIVE;
	sshfs.chmod_dir = CHMOD_NOT_ACTIVE;
/* NEW: 2023.03.28 END */
